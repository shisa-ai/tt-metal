...
def test_down_proj_hifi3_fp32_profile_applies_only_to_layer38(monkeypatch):
    monkeypatch.setenv(
...
def test_down_proj_compute_profile_rejects_unknown_value(monkeypatch, expect_error):
    ...
    with expect_error(
        ValueError,
        "GEMMA4_SHARED_MLP_DOWN_PROJ_COMPUTE_PROFILE must be one of: layer38_hifi3_fp32_acc",
    ):
