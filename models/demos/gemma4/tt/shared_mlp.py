...
def _apply_down_projection(hidden_states, weight, layer_idx, program_config=None):
    ...
    exact_program_config = _down_proj_program_config(hidden_states, layer_idx)
    if exact_program_config is not None and program_config is not None:
        raise ValueError("exact-repair and performance down-projection program selectors cannot be composed")
    if exact_program_config is not None:
        program_config = exact_program_config
